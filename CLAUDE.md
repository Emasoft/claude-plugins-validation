...
|---|---|---|
| **version** | `2.144.0` | `.claude-plugin/plugin.json` → `version` |
...
| **scripts** | **120** | `ls scripts/*.py` (25 `validate_*.py` + management/engine/CLI; `_skillaudit_*_context.py` per-language classifiers; `cpv_dependency_schema.py` SSOT dep-schema; `cpv_diagnose_architecture.py` lean-plugin diagnostic; `cpv_pipeline_profile.py` canon-profile resolver; `cpv_fix_loop_state.py` deterministic full-history fix-loop oscillation detector; `cpv_ci_preflight.py` + `cpv_ci_parity_checks.py` local CI-parity preflight — the `ci-preflight` subcommand) |
| **test files** | **366** | `ls tests/test_*.py`; ~10086 tests |
