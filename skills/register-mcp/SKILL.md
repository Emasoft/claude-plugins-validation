---
name: register-mcp
description: Register a new MCP server in an existing plugin's .mcp.json (stdio default; supports HTTP transport via --http-url; cross-platform command via Python/Node). Use when adding a new MCP server entry. Loaded by cpv-main-menu-agent.
