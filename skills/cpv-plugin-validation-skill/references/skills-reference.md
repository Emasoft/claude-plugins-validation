...
Claude Code includes a set of bundled skills that are available in every session, including `/code-review` (renamed from `/simplify` in v2.1.146), `/batch`, `/debug`, `/loop`, and `/claude-api`. Unlike most built-in commands, which execute fixed logic directly, bundled skills are prompt-based: they give Claude detailed instructions and let it orchestrate the work using its tools. You invoke them the same way as any other skill, by typing `/` followed by the skill name.
...
| `background`               | No          | Only applies with `context: fork`. Default `true` — the fork runs in the BACKGROUND, so the invoking turn gets an agent handle rather than the skill's result, and the fork runs with the narrower background-subagent tool set. Set `false` to wait for the result in the invoking turn. Requires v2.1.218 or later; before it, forked skills always blocked. |
