...
Within a single source, JSON object key uniqueness is enforced by the JSON parser, so duplicates can only occur ACROSS sources. Fix by removing the duplicate entry from one source — see [mcp-fixes.md §13](../../cpv-fix-validation/references/mcp-fixes.md#13-cross-source-duplicate-server-names).
