---
name: cpv-standardize-plugin
description: >
  Audit and fix plugin/marketplace repos to match CPV standards.
  Use when standardizing or auditing repo structure. Used dynamically via cpv-the-skills-menu (TRDD-478d9687).
...
3. **After --fix**, manually fix remaining issues:
   - **Under `--force-templates`** (the canon UPGRADE verb, NOT plain `--fix`), ALL agents are migrated to cpv-the-skills-menu: each agent's frontmatter `skills:` is rewritten to `[cpv-the-skills-menu]` (every other field preserved) and the mandatory dynamic-loading instruction is inserted into its body; a per-plugin `skills/cpv-the-skills-menu/SKILL.md` catalog is created if absent (reusing the scaffold generator so it is byte-identical). The migration is idempotent (re-running is a no-op) and skips + reports any agent file lacking YAML frontmatter — see the `cpv-the-skills-menu-create` skill for the canonical rewrite rules. Plain `--fix` never touches an agent.
   ...
   - **Empirical-loading-bugs MAJORs** (added 2026-04-18) need manual fixes — see `skills/cpv-canonical-pipeline/references/detailed-standard.md` "Empirical Validation Rules":
   ...
   - **Current pipeline standards** — load `cpv-fix-validation` skill's `pipeline-migration.md` for the full conversion recipes:
...
- [ ] `--fix` applied
- [ ] **For `--force-templates` runs**: every agent migrated to cpv-the-skills-menu (frontmatter `skills:` → `[cpv-the-skills-menu]` + dynamic-load body instruction) and `skills/cpv-the-skills-menu/SKILL.md` catalog present — see the `cpv-the-skills-menu-create` skill for the canonical rewrite rules
- [ ] Remaining issues fixed manually
- [ ] Re-validation passed
- [ ] **For migration runs (`/cpv-upgrade-plugin`)**: the 87-check matrix in the repo-root `references/canonical-pipeline-migration-checklist.md` (NOT inside this skill dir) returns exit 0 (every BLOCKER + MAJOR passes), AND a real `publish.py --patch` + `gh run watch --exit-status` returned green CI on the resulting tag. See the repo-root `agents/cpv-plugin-fixer-agent.md` "Pre-completion verification (REQUIRED)". Closes [issue #21 ask #1](https://github.com/Emasoft/claude-plugins-validation/issues/21).
...
- [Parallel scanning (v2.103.0+)](../cpv-canonical-pipeline/references/parallelism.md) — `cpv-standardize-plugin` re-runs validators after every fix; the v2.103.0+ rewrite makes each `validate_plugin` pass ~11.6× faster, so multi-iteration audits cost a fraction of pre-rewrite
