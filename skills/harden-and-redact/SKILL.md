---
name: harden-and-redact
description: >
  Catalog for fixing security-scan LEAK and MISSING-SAFEGUARD findings
  without loss of functionality — redacting secrets (and runtime-reading
  the needed ones from env / GitHub vars / OS keychain), and implementing
  missing safeguards: safe config parsing, input sanitization, correct
  launch / deploy / untrusted-input parameters, safe file loading, and a
  by-code-only prompt-injection pre-scan that runs before any agent reads
  untrusted content. Maps each finding to a before/after rewrite plus a
  no-functionality-loss note and a re-scan. Use when a security scan
  reports leaked secrets or missing safeguards that must be redacted or
  hardened rather than suppressed. Used dynamically via the-skills-menu
  by the plugin-leaks-preventer agent.
...
This is the leaks-and-hardening counterpart to the `devitalize-threats`
catalog. Devitalize neutralizes the *executable shape* of a shipped
threat; harden-and-redact removes *exposed data* and adds *missing
...
- A security report from `validate_security.py` + native skillaudit
  (the plugin-leaks-preventer agent produces it via the isolation
  launcher).
...
noted with a pointer to the `plugin-devitalizer` agent — the two jobs are
complementary.
