---
name: cpv-deterministic-codemod
description: "Deterministic codemod CLI — bulk-fix backtick-path to markdown-link, add TOC stubs, dedup blank lines, and other mechanical text transforms (issue 17). Zero LLM cost. Use when mechanical fixes outnumber semantic ones. Used dynamically via cpv-the-skills-menu (TRDD-478d9687)."
when_to_use: When the cpv-main-menu user picks Fix → Deterministic codemod, or any flow needs zero-LLM-cost mechanical transforms (backtick->link, TOC stubs, blank-line dedup) instead of dispatching the cpv-plugin-fixer-agent agent
...
4. Re-validate via `cpv-plugin-validation-skill` and confirm finding counts dropped.
...
- [Subcommands](references/subcommands.md)
  > Subcommand table · Safety contract · Recommended workflow · When the codemod is the WRONG tool · Recovery
- `cpv-plugin-validation-skill` — find what to fix (read-only)
- `cpv-fix-validation` skill — agent-driven fixes for findings that require judgment
