...
Detailed semantic-validation rules for the **Channel MCP Server Source-Code Security** pillar. This reference is loaded by the `cpv-semantic-validator-agent` agent whenever a plugin declares a non-empty `channels` array in `plugin.json`.
...
- `skills/cpv-fix-validation/references/plugin-error-index.md` — error catalogue (structural channel errors)
- `skills/cpv-fix-validation/references/skill-semantic-validation.md` — parent grading rubric
