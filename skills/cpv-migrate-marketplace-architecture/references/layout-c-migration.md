...
- **A Layout A or B configuration** → use `cpv-migrate-marketplace-architecture` to walk through the full restructure (out of scope for this reference).
...
Skip the entire `cpv-setup-marketplace-auto-notification` flow for Layout C migrations.
