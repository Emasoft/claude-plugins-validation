...
This reference is loaded by the `cpv-migrate-marketplace-architecture` skill after
...
`skills/cpv-setup-marketplace-auto-notification/references/` and that skill is the
...
1. **Check the shell env for a PAT** and set the marketplace secret via the
   ...
   **FORBIDDEN patterns** — reject these on sight, do not emit them:
   ...
   `../../../cpv-setup-marketplace-auto-notification/references/pat-secret-setup.md`.

2. **Scaffold `.github/workflows/notify-marketplace.yml`** on the plugin repo
   using the template at
   `../../../cpv-setup-marketplace-auto-notification/references/notify-workflow-template.md`
