...
Loaded by the `cpv-migrate-marketplace-architecture` skill when the user selects
...
That is the job of the `cpv-setup-marketplace-auto-notification` skill — PAT,
...
- **No PAT required.** `MARKETPLACE_PAT` is not used by Layout B. The env var
  ...
  `cpv-setup-marketplace-auto-notification` skill is NOT loaded for Layout B
  migrations.
...
3. Install the marketplace receiver workflow from
   `../../../cpv-setup-marketplace-auto-notification/references/receiver-workflow-template.md`
...
mkdir -p "$MAIN_ROOT/reports/cpv-migrate-marketplace-architecture"
uv run --with pyyaml python scripts/validate_marketplace.py . --strict \
  2>&1 | tee "$MAIN_ROOT/reports/cpv-migrate-marketplace-architecture/$(date +%Y%m%d_%H%M%S%z)-layout-b-verify.log"
```
...
`$MAIN_ROOT/reports/cpv-migrate-marketplace-architecture/<YYYYMMDD_HHMMSS±HHMM>-<slug>.md`
