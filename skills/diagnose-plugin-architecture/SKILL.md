---
name: diagnose-plugin-architecture
...
Do NOT use it to perform the separation — that is the `strip-dev-submodules`
...
| User asks to APPLY a fix | This skill is advisory — hand off to `strip-dev-submodules` / manual `.gitignore` / hook edits; do not mutate the plugin here. |
...
- `strip-dev-submodules` skill — the SEPARATE engine that performs the submodule extraction this skill recommends.
