---
name: add-component-to-plugin
description: Add a new component (skill / agent / command / hook / mcp) to an existing plugin. Use when scaffolding a single component into a plugin without re-running the generator. Used dynamically via skills-index (TRDD-478d9687).
...
4. After scaffolding, refresh the README via the `refresh-readme` skill so the `<!-- BEGIN AUTO-COMPONENTS -->` block picks up the new component.
5. Re-validate the parent plugin via `plugin-validation-skill` to confirm no drift was introduced.
...
- `scaffold-skill` skill — focused single-purpose skill wrapper
- `scaffold-agent` skill — focused agent scaffold wrapper
- `scaffold-command` skill — focused command scaffold wrapper
- `add-hook` skill — focused hook merge wrapper
- `register-mcp` skill — focused MCP registration wrapper
- `refresh-readme` skill — refresh the auto-components README block
