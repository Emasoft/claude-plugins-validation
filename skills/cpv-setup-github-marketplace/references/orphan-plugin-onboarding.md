...
2. Create the layout:
   ...
   ```
3. Validate via the launcher (NEVER call `validate_marketplace.py` directly from the cache — the environment-isolation guard refuses): `cpv-remote-validate marketplace <marketplace-folder> --strict`. Route above-WARNING findings to the `cpv-marketplace-fixer-agent` agent.
4. Validate the plugin itself is still clean via the same launcher: `cpv-remote-validate plugin <plugin-folder> --strict`. Route to `cpv-plugin-fixer-agent` if needed.
...
2. **Plugin repo** (Layout A): run the full "Publish Plugin as GitHub Repo" workflow from `agents/cpv-plugin-creator-agent.md` step-by-step. This gives the plugin its own GitHub repo with `scripts/publish.py`, CI, pre-push hooks, first tag + GitHub release.
3. **Marketplace repo**: invoke the `cpv-setup-github-marketplace` skill's setup guide. Create `<owner>/<marketplace>`, install `marketplace.json` + CI/CD templates + `update-submodules.yml` dispatch receiver.
4. **Link the plugin**: invoke the `cpv-publish-to-marketplace` skill to wire `notify-marketplace.yml` on the plugin repo + set `MARKETPLACE_PAT` via `scripts/set_marketplace_pat.py` (never pipe to `gh secret set`).
...
Every step of this path is covered by existing skills: `cpv-setup-plugin-repo`, `cpv-canonical-pipeline`, `cpv-setup-github-marketplace`, `cpv-publish-to-marketplace`, `cpv-setup-marketplace-auto-notification`. Load them on demand — don't improvise.
...
2. Validate the target: `cpv-remote-validate marketplace <owner>/<marketplace> --strict`. If findings above WARNING, route to `cpv-marketplace-fixer-agent` before continuing — don't register into a broken marketplace.
