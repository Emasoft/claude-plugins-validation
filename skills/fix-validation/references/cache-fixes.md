...
- [CA-04 — SKILL.md `model:` frontmatter forces in-line model switch](#ca-04--skillmd-model-frontmatter-forces-in-line-model-switch)
...
## CA-04 — SKILL.md `model:` frontmatter forces in-line model switch

| Field | Value |
|---|---|
| **Severity** | MINOR |
| **Validator** | `validate_cache.py` |
| **Triggered by** | A `model: opus` (or other override) in a SKILL.md frontmatter block |
| **Why it matters** | An in-line model switch mid-prompt forces a fresh prompt-cache key for the new model. If the surrounding session is on Sonnet and the skill says `model: opus`, every invocation pays a full Opus cache miss |

### Fix

1. Remove the `model:` frontmatter from SKILL.md unless the skill genuinely cannot work on the session model
2. If the skill MUST run on a different model, refactor it as an AGENT instead — agents own their model context and can invoke the skill as part of their isolated turn without forcing the parent session to switch

```yaml
# WRONG — forces a Sonnet-session miss every invocation
---
name: heavy-analysis-skill
model: opus
---

# RIGHT — let the agent that loads this skill choose the model
---
name: heavy-analysis-skill
description: Deep semantic analysis (use via opus agent for best results).
---
```
