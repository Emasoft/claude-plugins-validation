...
name: fix-validation
description: |
  Master index mapping CPV validation errors to fix reference files in references/.
...
Central lookup for the fix agent. Given a validation error, find the reference file with fix instructions. All fix guides are in `references/` and have a TOC in their first 200 chars.

## Prerequisites

- A validation report file generated by `/cpv-validate-plugin` or `/cpv-validate-skill`
- Access to the `references/` directory containing fix guide `.md` files
...
Error: `[MAJOR] Missing plugin.json` → Look up validate_plugin.py → Open `references/plugin-structure-fixes.md` → §1. Plugin Manifest Issues.

Error: `[MAJOR] Description too short` → Look up validate_skill_comprehensive.py → Open `references/skill-fixes.md` → §4. Description Quality Issues.

## Resources

All fix guides: `references/*-fixes.md`
Semantic validation: `references/skill-semantic-validation.md`
Detailed procedures: `references/plugin-validator-detailed-procedures.md`
