# Canonical Launcher Invocation (plugin alias)
...
mkdir -p "$MAIN_ROOT/reports/validate_plugin"
REPORT="$MAIN_ROOT/reports/validate_plugin/$(date +%Y%m%d_%H%M%S%z)-<slug>.md"

CLAUDE_PRIVATE_USERNAMES="$(whoami)" uv run --with pyyaml \
  python "${CLAUDE_PLUGIN_ROOT}/scripts/remote_validation.py" \
  plugin "<plugin_path>" --strict --report "$REPORT"
```

## Why the launcher is mandatory

CPV's `validate_plugin.py` refuses direct invocation from the plugin
