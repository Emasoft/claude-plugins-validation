...
- [ ] Refactor between layouts only via `migrate-marketplace-architecture`
...
1. In the plugin repo, add `.claude-plugin/marketplace.json` mirroring the plugin's identity (see `migrate-marketplace-architecture` → `layout-c-migration.md`).
...
4. Set up the auto-notify chain (`setup-marketplace-auto-notification`).
