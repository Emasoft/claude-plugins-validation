...
**Preconditions**: user owns `<owner>/<marketplace-repo>` on GitHub (create if absent), marketplace layout is already Layout B (`./plugins/<name>` relative sources) OR the agent is authorized to migrate Layout A → Layout B via `marketplace-fixer`.

**Steps**:

1. Validate every plugin: loop `validate_plugin.py --strict` over each subfolder. Fix anything non-WARNING via the `plugin-fixer` agent.
2. Validate the marketplace: `validate_marketplace.py --strict`. Fix via `marketplace-fixer` if needed.
...
6. Install the Layout-B publish pipeline from `canonical-pipeline` skill (one `scripts/publish.py`, one `cliff.toml`, one `CHANGELOG.md` at marketplace root).
...
1. For each plugin subfolder: validate (`plugin-fixer` if needed), git init inside the plugin subfolder, apply `generate_plugin_repo.py` templates for CI/CD + publish pipeline.
...
8. Wire per-plugin auto-notify: for each plugin, install `notify-marketplace.yml` targeting the marketplace repo AND set `MARKETPLACE_PAT` via `scripts/set_marketplace_pat.py <owner>/<plugin> <owner>/<marketplace>`. This is the setup the `setup-marketplace-auto-notification` skill automates — load it for this phase.
