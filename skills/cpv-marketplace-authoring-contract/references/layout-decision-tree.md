...
- Auto-notification setup (CPV's `cpv-setup-marketplace-auto-notification`) needed to keep marketplace.json fresh on plugin releases.
...
Layout migrations are tracked in detail in `skills/cpv-migrate-marketplace-architecture/`. The decision points:
...
Never migrate without running CPV's pre-migration audit (the cpv-migrate-marketplace-architecture skill's first step).
...
When the user asks "create a marketplace" via cpv-plugin-creator-agent:
...
- `skills/cpv-migrate-marketplace-architecture/` — sister skill for transitioning between layouts
