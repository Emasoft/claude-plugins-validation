...
4. **Install git hooks**: run `uv run python scripts/publish.py --install-hook` (copies `git-hooks/pre-push` to `.git/hooks/` and sets `core.hooksPath`). The pre-push hook is a thin bash delegator to `publish.py --gate` — it runs 4 gates (version bump, lint, validate --strict, tests) and blocks pushes with ANY non-WARNING issue.
