# Batch-fix protocol — cpv-plugin-fixer-agent agent modes
...
`cpv-plugin-fixer-agent` agent follows when dispatched in a batch mode. The three
...
3. **MECH first, then INTEL fix-as-you-go.** Auto-apply the deterministic set for
   ...
   Route recipes via the `cpv-fix-validation` error-to-fix mappings (open a rule-TYPE's
