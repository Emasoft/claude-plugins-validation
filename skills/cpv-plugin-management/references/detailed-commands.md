...
Interactive entry point: `/cpv-main-menu` → **Create** (scaffold plugin / marketplace / skill / agent / command / hook / MCP) and the publishing flows route through it (it dispatches the cpv-plugin-creator-agent agent). The individual `/cpv-create-*` / `/cpv-publish-*` / `/cpv-standardize` slash commands were consolidated into `/cpv-main-menu` and no longer exist.
