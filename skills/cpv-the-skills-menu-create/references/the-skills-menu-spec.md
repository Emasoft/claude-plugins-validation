# cpv-the-skills-menu-create — universal skill-discovery method specification
...
**static agent skill assignment** to **cpv-the-skills-menu method** —
agents declare only one preloaded skill (`cpv-the-skills-menu`) and pick
operational skills dynamically at runtime via the `Skill()` tool.

This skill creates the `cpv-the-skills-menu` skill inside the target
plugin and rewrites all of its agents.

After this skill runs, every agent in the target plugin must:

1. have only `cpv-the-skills-menu` in its frontmatter `skills:` list;
2. contain the mandatory dynamic skill-loading instruction in its body;
3. rely on the generated `cpv-the-skills-menu` skill to discover and load
...
```text
/cpv-the-skills-menu-create create a skills menu from my-example-plugin
/cpv-the-skills-menu-create create a skills menu from the plugin project in ~/code/projects/my-scraping-plugin/
/cpv-the-skills-menu-create create a menu from my-example-plugin contained in github.com/example-org/example-marketplace
/cpv-the-skills-menu-create scaffold the menu for the plugin at /tmp/scratch/my-plugin
```
...
user, no agents). Still generate `cpv-the-skills-menu/SKILL.md` as a
...
**Do NOT list `cpv-the-skills-menu` itself as an entry inside its own
...
`cpv-the-skills-menu` folder itself (i.e. no OTHER skills to catalog),
...
```text
<plugin-root>/skills/cpv-the-skills-menu/SKILL.md
```
...
description: "Dynamic skill menu for this plugin. Teaches agents which skills are available, when to use them, and how to load them with the Skill() tool. Use when an agent needs to pick a downstream skill at runtime. Used by every agent in this plugin via cpv-the-skills-menu method."
...
The generated `cpv-the-skills-menu/SKILL.md` is a real skill and MUST pass
...
into `skills/cpv-the-skills-menu/references/skills-catalog.md` and keep a
...
- never blindly overwrite an existing `cpv-the-skills-menu/SKILL.md` without
  inspecting it first (it may be a hand-curated catalog).

If `cpv-the-skills-menu` already exists, decide by content shape:
...
- that entry is `cpv-the-skills-menu`;
- every agent body contains the mandatory dynamic-loading instruction;
- `<plugin-root>/skills/cpv-the-skills-menu/SKILL.md` exists;
...
### Verification
- [x] cpv-the-skills-menu exists at <plugin-root>/skills/cpv-the-skills-menu/SKILL.md
- [x] all agent frontmatters use only cpv-the-skills-menu
...
from the generated `cpv-the-skills-menu` instead of loading large static
