...
- [ ] Architecture signal? → route to `cpv-migrate-marketplace-architecture` skill instead
...
For a full interactive Layout A (hub-and-spoke: split each plugin into its own repo) or Layout B (nested with full CPV discipline) migration, use [`skills/cpv-migrate-marketplace-architecture/`](../../cpv-migrate-marketplace-architecture/SKILL.md). That skill owns the large-scale conversion workflow with user prompts, git subtree splitting, and repo creation.
...
1. Copy the template from any CPV-managed plugin (for example `skills/cpv-canonical-pipeline/references/cliff.toml`) or start from the [git-cliff docs](https://git-cliff.org/docs/configuration).
...
For a more complete CI setup (linting, mypy, pytest, release automation), see [`skills/cpv-canonical-pipeline/`](../../cpv-canonical-pipeline/SKILL.md).
...
For a production-grade `publish.py` with lint/test/validate gates, pre-push hook integration, and GitHub Release creation, see [`skills/cpv-canonical-pipeline/`](../../cpv-canonical-pipeline/SKILL.md).
...
2. Tell their authors to publish independently — they can use [`skills/cpv-setup-plugin-repo/`](../../cpv-setup-plugin-repo/SKILL.md) for their own repos.
3. Commit the pruned `marketplace.json`.

For interactive Layout A migration (split each plugin into a separate repo automatically), use the `cpv-migrate-marketplace-architecture` skill.
...
2. Use the `cpv-migrate-marketplace-architecture` skill for the full interactive migration.
3. Update `marketplace.json` to reference each plugin via `{ "source": { "source": "github", "repo": "owner/plugin-a" } }`.

See `skills/cpv-migrate-marketplace-architecture/` for the full Layout A / Layout B migration procedures — this section only covers the simpler mechanical fixes.
