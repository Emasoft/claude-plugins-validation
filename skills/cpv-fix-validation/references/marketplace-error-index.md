...
- [ ] Distinguish MECHANICAL findings (route to `cpv-fix-marketplace-validation`) from ARCHITECTURE signals (route to `cpv-migrate-marketplace-architecture`)
...
For a full interactive Layout A (hub-and-spoke: git subtree split each plugin to its own repo) or Layout B (nested with CPV discipline) migration, use the `cpv-migrate-marketplace-architecture` skill invoked by the `cpv-marketplace-fixer-agent` agent. The per-warning mechanical fixes in marketplace-fixes §9 handle the simpler case where you just want to bring an existing layout into compliance without moving repositories around.
...
- `skills/cpv-create-plugin/references/marketplace-layouts.md` for manual migration
- The `cpv-marketplace-fixer-agent` agent with `/cpv-fix-marketplace-validation <report.json>` for automated conversion
