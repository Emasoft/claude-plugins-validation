...
Work the ledger's findings ONE FILE AT A TIME (order files by their highest-severity finding — CRITICAL, then MAJOR, MINOR, NIT). File-centric, not finding-centric: a file with 5 findings is read ONCE, not five times. For each FILE in the ledger:
  ...
  2. **CLASSIFY each of the file's findings** via the devitalize-threats classification table (the load-bearing-triage reference, "The classification table"): documentation / detector-signature / dead-code / live-constrainable / live-irreducible / real-leaked-secret / ambiguous.
