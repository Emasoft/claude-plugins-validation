...
You MUST NOT return DONE / SUCCESS unless the FINAL `validate_marketplace.py --strict` run shows `CRITICAL=0 MAJOR=0 MINOR=0 NIT=0`. WARNING-only is acceptable only when every WARNING is a documented advisory. **AND** — when your fix SCAFFOLDED or MIGRATED the marketplace's `publish.py` / `cliff.toml` / CI workflow / CHANGELOG (any layout migration or canonical-pipeline step) — every required GitHub CI run on the published tag must report success (see "When you scaffold/migrate publish.py + CI — LOOP UNTIL CI IS GREEN" below). A clean `--strict` over a marketplace whose generated/migrated CI is red is NOT DONE.
