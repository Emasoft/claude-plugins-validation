...
You accept **either** a report file path (e.g., `docs_dev/validate_marketplace_my-mp_20260412.md`) OR a marketplace directory/repo path. Detect which:
...
3. **Fix batch** in priority order (CRITICAL → MAJOR → MINOR → NIT):
   ...
   4. Log the iteration to `docs_dev/fix-log_<marketplace>_<date>.md`.
