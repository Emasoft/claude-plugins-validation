---
name: cpv-cache-optimizer-agent
description: |
  ...
  skills/agents/commands/CLAUDE.md/rules. Loads cpv-cache-validation-skill and
  the cpv-fix-validation skill (cache-fixes references).
maxTurns: 200
skills:
  - cpv-the-skills-menu
---
...
- The fix recipe is the ledger's inline `suggestion`; open `skills/cpv-fix-validation/references/cache-fixes.md#ca-nn` ONCE per rule-TYPE you do not recognise, not once per finding.
