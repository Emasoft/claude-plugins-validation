...
9. Configure git hooks: **run this FROM INSIDE the newly-scaffolded plugin repo** (the one you just created in step 7-8) — `uv run python scripts/publish.py --install-hook`. Those `--install-hook` and `--gate` flags live in the `scripts/publish.py` that `generate_plugin_repo.py` writes into every new plugin (see `gen_publish_py` at `scripts/generate_plugin_repo.py:902`). **DO NOT** run this from inside CPV itself — CPV's own `scripts/publish.py` is the minimal release-bump variant without those modes and will error with `argparse: unrecognized arguments: --install-hook`. Once installed, the pre-push hook delegates to `publish.py --gate` which runs lint + validate (--strict) + tests and blocks pushes with ANY non-WARNING issue.

   **Pipeline standards baked into every newly-scaffolded plugin:**
   - **Pure Python with cross-platform libraries — no bash, no jq, no sed/awk** — every script shipped in a CPV-scaffolded plugin must run identically on Linux, macOS, AND Windows. Bash scripts (`.sh`) and tools that aren't natively packaged on Windows (`jq`, `shellcheck`) are publish-blockers. **Python scripts must use cross-platform libraries throughout: `pathlib.Path` (NOT `os.path.join`/`os.path.exists`/`os.path.isdir`), `tempfile.gettempdir()` (NOT hardcoded `"/tmp/"`), `shutil.which()` (NOT shell `which`), `subprocess.run([...], timeout=N)` (NOT `os.system`/`shell=True`).** The plugin-fixer's `pipeline-migration.md §3a` documents the conversion recipe for legacy plugins. Newly-scaffolded plugins NEVER ship `.sh` files.
   - **Every script-input parameter MUST be sanitized to prevent injection** — every CLI flag, env-var read, argv element, JSON field, file content, and gh-API response that flows into a subprocess call, a regex compile, an SQL query, an HTTP URL, a file path, or a shell argument must be validated against a strict regex/allowlist BEFORE use. Concrete rules: (a) NEVER `shell=True` — always pass `args` as a list; (b) repo slugs match `^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$` (see `set_marketplace_pat.py:REPO_PATTERN` for the canonical regex); (c) version strings match `^\d+\.\d+\.\d+(?:[-+][\w.]+)?$`; (d) plugin names match `^[a-z][a-z0-9-]*$` (kebab-case); (e) file paths are resolved with `Path(...).resolve()` and checked against the plugin root with `relative_to()` to reject `..`-traversal; (f) regex patterns from user input go through `re.escape()` before being compiled; (g) URLs from user input have their host validated against an allowlist BEFORE passing to `gh api` / `urlopen`. The validator scans for common unsanitized patterns and emits MAJOR/CRITICAL when detected.
   - **Whole-repo lint via cpv_lint_engine** — the validator owns linting. There is no separate per-language lint script (the legacy lint script was removed). Every supported language (Python, JS/TS, Rust, Go, Bash, Markdown, YAML, JSON, TOML, …) is linted in one pass via remote-exec fallback (`uvx`/`bunx`/`docker`). The CI workflow runs `scripts/cpv_lint_engine.py .`; the pre-push hook runs `validate_plugin.py` which calls into the same engine.
   - **Idempotent publish.py** — `stage_bump`, `stage_commit`, `stage_tag` all detect "already done" state and skip cleanly. An interrupted publish (push 503, network drop, hook reject) can be re-run with the same arguments and resumes from the failed gate without double-bumping or version drift. The bump baseline is `origin/master`'s `plugin.json.version`, NOT local — local being ahead just means the previous run was interrupted past the bump stage.
   - **validate_pipeline_script_refs rule** — `validate_plugin.py` scans every `.github/workflows/*.{yml,yaml}`, `.git/hooks/*`, `git-hooks/*` (source-of-truth templates), the `setup_plugin_pipeline.py` PRE_PUSH_HOOK template, and `plugin-validation-skill/references/*` for dangling `scripts/<name>.py` references. If a script is renamed/removed but a pipeline still calls the old name, the validator emits MAJOR with `file:line` so it's caught at PR/release time before any push attempts use the stale ref.
   - **Hook commands MUST NOT write to `${CLAUDE_PLUGIN_ROOT}`** — that directory is REPLACED on every plugin update (~7-day cleanup). State written there is silently destroyed. Plugins must persist data in `${CLAUDE_PLUGIN_DATA}` (the `~/.claude/plugins/data/<plugin-id>/` directory). The new `validate_hook` rule emits CRITICAL on any hook command that writes runtime state under `${CLAUDE_PLUGIN_ROOT}` (shell redirects, `tee`, `sqlite3`, package-manager `--prefix`/`--target` flags).
   - **Hook commands MUST be cross-platform (Python-delegated, no bash-isms)** — every hook command in `hooks/hooks.json`, plugin.json's inline `hooks`, AND agent / skill frontmatter `hooks:` runs on Linux, macOS, AND Windows. Bash-only constructs (`set -euo pipefail`, `[[ ]]`, `$(<file)`, process substitution `<(...)`, brace expansion `{a,b,c}`) emit MAJOR. POSIX-only tools used directly (`jq`, `sed`, `awk`, `shellcheck`) emit MINOR unless wrapped in `python3 -c "..."` / `bash -c "..."` / `wsl bash -c "..."`. The recommended pattern is to delegate every non-trivial hook to a Python script bundled under `${CLAUDE_PLUGIN_ROOT}/scripts/`. See `pipeline-migration.md §3b` for the conversion cheat-sheet.

   - **PEP 723 scripts MUST be invoked via `uv run`, NEVER bare `python`** — when a Python script under `scripts/` declares runtime dependencies via a PEP 723 inline-script metadata block (`# /// script ... # ///` with a non-empty `dependencies = [...]`), every reference to that script in commands / agents / skills / hooks / README / `.mcp.json` / `.lsp.json` / `hooks.json` MUST use `uv run scripts/<name>.py <args>` (or the equivalent `uv run --with <deps> python scripts/<name>.py` form). Bare `python scripts/<name>.py` IGNORES the inline metadata block and the script ImportErrors on the first non-stdlib import the moment a user runs it — the plugin "looks valid" to every static check yet is broken at runtime. CPV's `validate_pep723_invocations` flags every offending invocation as MAJOR `[RC-PEP723-INVOCATION-001]`. The cpv-codemod `python-to-uv-run` transform applies the fix in bulk. **When emitting any PEP 723 script during scaffolding, ALWAYS double-check the README / commands / agents / skills you're generating in the same pass to ensure every invocation uses `uv run`.**
