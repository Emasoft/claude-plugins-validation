...
description: |
  ...
  (cpv-plugin-fixer-agent, cpv-cache-optimizer-agent, etc.).

  Bundled into the plugin as part of v2.103.2 so CPV's dispatch chain
  no longer depends on a user-scope agent — every install gets cpv-spark-agent
...
specialised CPV work agent (cpv-plugin-fixer-agent, cpv-cache-optimizer-agent,
cpv-marketplace-fixer-agent) instead.
...
```text
$MAIN_ROOT/reports/cpv-spark-agent/{timestamp}-{slug}.md
```
...
5. **Know limits** — escalate to a specialised CPV work agent
   (cpv-plugin-fixer-agent, cpv-cache-optimizer-agent, cpv-marketplace-fixer-agent) if the
...
assistant: Bounded single-file fix — exactly cpv-spark-agent's lane.
...
[DONE] fix-readme-langtag — 1 warning cleared. Report: reports/cpv-spark-agent/20260525_142210+0200-fix-readme-langtag.md
</example>
...
[DONE] batch-shard-3 — 4 MINOR findings cleared across 4 files. Report: reports/cpv-spark-agent/20260525_144705+0200-batch-shard-3.md
</example>
