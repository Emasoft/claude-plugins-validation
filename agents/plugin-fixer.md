...
**CI-parity preflight (applies to EVERY run, not just migration).** `validate_plugin --strict` does NOT mirror CI's Lint job (jscpd/actionlint/mypy/`uv sync --extra dev`) — the #137-143 root cause. So if ANY fix in this run touched `.github/workflows/*`, `pyproject.toml`, or `publish.py`, run `remote_validation.py ci-preflight <plugin-root>` (the launcher's `ci-preflight` subcommand) and resolve every non-WARNING finding (CIP-1..5 + the live gates) BEFORE declaring DONE. A `ci-preflight` WARNING (a tool is absent on the box) does NOT block — it degrades; a real over-threshold / static-defect / resolve-failure is a fix iteration, not a stop. If no fix touched those paths, skip this.
...
1. **Validate** via the launcher `remote_validation.py plugin <plugin-root> --strict --report <tmp.md> --json <findings.json>` (NEVER call `validate_plugin.py` directly — the isolation guard refuses with a "remote location" error; aliases: runbook §13); read the report + `SUMMARY:` line. Then `uv run scripts/cpv_fix_loop_state.py record --state <loopstate.json> --findings <findings.json>` → `CONVERGED` (go to step 7), `PROGRESS` (fix a batch, step 2), or `CYCLE` (oscillation — see the termination paragraph below; do NOT re-apply the same fix).
...
4–6. **If empty** → split WARNINGs into `blocking` / `advisory` (categories table in `iterative-fix-loop.md` — that file is supporting DATA, not the loop logic); fix blocking ones (go to 1); only-advisory → step 7.
7. **MANDATORY FINAL VERIFICATION** — run the validator ONE MORE TIME as a clean-room re-check (independent of the loop's exit state); its output is what you return. ANY non-WARNING finding → back to step 1. Non-skippable.
   ...
   **7d (migration ONLY) — publish, then LOOP UNTIL CI IS GREEN** — after 7 + 7c pass, run `publish.py --patch` then `gh run watch <run-id> --exit-status` on the tag (repeat for a Layout-C/external marketplace). A red run is the NEXT fix iteration, not a stop: read the failing job (`gh run view`), fix the CAUSE on the plugin side (failing test/lint/type/permission — NEVER mute the check, NEVER `--force-templates`), re-publish, re-watch — until every required run is green. Track the failing-job set with a SECOND state file (`cpv_fix_loop_state.py … --state <ci-loopstate.json>`); return `[PARTIAL]` ONLY when that set oscillates (a CI fix is not landing, or the failure is environmental), citing the `gh run view` URL. Each re-publish is a real `--patch` bump (correct — every attempt is an auditable release attempt). GitHub TRANSIENT failures (network/runner) are re-run with `gh run rerun --failed`, not counted as a fix-cycle.
