...
description: |
  ...
  cpv-fix-validation, cpv-canonical-pipeline, and cpv-plugin-validation-skill skills on
  demand. For cpv-canonical-pipeline migration it ALSO enforces the 87-check
...
| 2 | `0 < total_findings ≤ safe-ceiling` | Enter the loop; load `cpv-fix-validation`. |
...
| 4 | mode `batch_shard` | Enter §"Batch modes"; load `cpv-batch-fix-protocol` + `cpv-fix-validation`. |
| 5 | mode `cpv-canonical-pipeline migration` | Enter §"Migration exit contract"; load `cpv-canonical-pipeline`. |
| 6 | mode `marketplace fix` | Refuse: `[BLOCKED] wrong-agent — use cpv-marketplace-fixer-agent`. |
...
|---------|--------------|
| **standard** | Current behavior — the loop + `cpv-fix-validation` recipes apply as-is. |
...
Triage report: <plugin-root>/reports/cpv-plugin-fixer-agent/<timestamp>-triage.md
```
...
> **Migration is NOT complete until:** **(a)** every BLOCKER + MAJOR in the 87-check matrix [`references/canonical-pipeline-migration-checklist.md`](../references/canonical-pipeline-migration-checklist.md) passes, **AND (b)** a real `publish.py` run reports green CI on the tag, **AND (c)** any Layout-C / external marketplace the plugin is registered in reports green CI on its tag, **AND (d)** every `agents/*.md` is on cpv-the-skills-menu (`skills: [cpv-the-skills-menu]` + the dynamic-load body instruction) — performed by `standardize_plugin.py --fix --force-templates` (which now migrates every agent) or an explicit `cpv-the-skills-menu-create` run. This (d) is CONDITIONAL canon: it is required ONLY on the migrate/upgrade/publish path (this contract), never on a plain user `validate` — the validator stays silent about cpv-the-skills-menu otherwise. **Any failure** → report `[PARTIAL]` with the failed `CHECK-NN` rows (`file:line`) and **stop**; do NOT auto-rerun publish.py or silently `--force-templates`.
...
When the `<context>` block carries a `mode:` of `batch_shard`, `batch_per_plugin`, `batch_same_turn_validate_fix`, or `batch_same_turn_full`, you are one of N parallel fixers dispatched by the batch slash commands (`/cpv-batch-fix` etc.). **Load `cpv-batch-fix-protocol` and follow its `agent-modes.md` reference for the authoritative per-mode workflow, scope-ownership rules, status-JSON shapes, and one-line return formats** (`index.json` / shard-manifest / shard-status schemas: the same skill's `json-schemas.md`).
...
7. **MANDATORY FINAL VERIFICATION** — run the validator ONE MORE TIME as a clean-room re-check (independent of the loop's exit state); its output is what you return. ANY non-WARNING finding → back to step 1. Non-skippable.
   ...
   **7c (migration ONLY)** — run the 87-check matrix (§Pre-completion verification); a BLOCKER/MAJOR fail = a CRITICAL/MAJOR finding → return `[PARTIAL]` with each failed `CHECK-NN` and do NOT proceed. The matrix now includes **CHECK-83..87** (the `ci-preflight` CI-parity gate — CIP-1..8 + the live jscpd/actionlint/mypy/dev-extra/cspell gates), so a `--force-templates`-clean migration that passed `validate_plugin --strict` can no longer silently RED-CI on a #137-143 shape — it is caught here, before 7d's real publish.

   **7d — WHENEVER YOU PUBLISH: publish, then LOOP UNTIL CI IS GREEN (unconditional, not migration-only).** "DONE" means green CI, not "files written". `publish.py --patch` → `gh run watch <run-id> --exit-status` on the tag (repeat for a Layout-C/external marketplace). A red run — OR a `cancelled` required run (never a pass; e.g. a leftover `validate.yml` concurrency-cancelled by `ci.yml` → remove it, CIP-4) — is the NEXT iteration: read the failing job (`gh run view --log-failed`), fix the CAUSE plugin-side (NEVER mute the check / `--force-templates`), reproduce + fix LOCALLY, re-publish, re-watch until every required run is green. **LEAN-CAPTURE all publish/pytest/CI output to a file, read back ONLY the failure summary** — never the raw stream (cost ≈ turns × per-turn-context — the 16-25M-token burn fix, TRDD-DZS5K34A). Bound the cycles with a SECOND state file (`cpv_fix_loop_state.py … --state <ci-loopstate.json> --stall-window 5`); return `[PARTIAL]` on `CYCLE` (exact-repeat) OR `STALLED` (no new-best failing-job count for 5 release+CI cycles), citing the `gh run view` URL. Transient (network/runner) failures → `gh run rerun --failed`, not a fix-cycle. Full detail — lean-capture recipe, stall math, cancelled-vs-pass: `iterative-fix-loop.md` §"Publish / upgrade — loop until CI green" + §"Token discipline".
...
- **ALWAYS write reports/fix logs to `$MAIN_ROOT/reports/cpv-plugin-fixer-agent/<YYYYMMDD_HHMMSS±HHMM>-<slug>.md`** (`$MAIN_ROOT` = main-repo root via `git worktree list | head -n1 | awk '{print $1}'`, never a linked worktree; `reports/`+`reports_dev/` gitignored). The log holds the iteration history, per-batch diffs, and advisory-warning list; return only a one-line summary.
...
[DONE] fixed 3 of 3 issues. Report: reports/cpv-plugin-fixer-agent/20260421_184530+0200-my-plugin.md
</example>
...
[DONE] iterations=3, clean. Report: reports/cpv-plugin-fixer-agent/20260421_191205+0200-my-plugin.md
</example>
