---
name: cpv-plugin-diagnoser-agent
description: |
...
specialised agent (cpv-plugin-fixer-agent, cpv-marketplace-fixer-agent, cpv-plugin-creator-agent) only
...
Run [shape-detection](../skills/cpv-plugin-validation-skill/references/shape-detection.md)
...
`validate_plugin.py` pass — those dispatches (cpv-plugin-creator-agent marketplace-mode /
...
Load `cpv-fix-validation` and read its pipeline-migration.md reference (§0 canonical-drift · §0b legacy-pipeline-scripts · §1 dangling refs · §2 whole-repo lint · §3 cross-platform Python · §4 idempotent publish.py · §5 input sanitization), then run its per-section detection commands. §0/§0b are surfaced from
...
Route the chosen key per the dispatch table: F/C/J → cpv-plugin-fixer-agent at
`min_severity` WARNING/CRITICAL/MAJOR (with the pipeline-migration §1–§5
prompt); R → cpv-plugin-creator-agent in marketplace-mode; S → confirm + run
...
Report: $MAIN_ROOT/reports/cpv-plugin-diagnoser-agent/<ts>-<plugin>.md
```
