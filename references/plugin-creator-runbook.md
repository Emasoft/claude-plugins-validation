# plugin-creator runbook

Detailed reference loaded by the `plugin-creator` agent (`agents/plugin-creator.md`).
...
For destinations 1/2/3: copy the folder (or `ln -s` with user consent) there, then run `validate_skill.py` (or route to `skill-validation-agent`) and report. For destination 4: fresh scaffold — wrap, move the skill to `<plugin-root>/skills/<name>/`, then enter the deploy flow.
