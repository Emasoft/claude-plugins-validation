...
RUNNING: Unit-C (standardize_plugin.py `migrate_agents_to_skills_menu` under --force-templates, per the-skills-menu-spec.md SSOT + reusing gen_the_skills_menu_skill; + standardize-plugin SKILL doc + two-sided tests).
...
**NEXT ACTION:** await Unit-C → central-verify (ruff + mypy scripts/ + new tests + cache-cold self-validate, regen hashes LAST) → ship Increment B → Increment C (Gap B) → Phase 4 docs/README.
