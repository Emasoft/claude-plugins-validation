...
argument-hint: "<action> <plugin>@<marketplace> [--scope user|local]"
...
```bash
uv run "${CLAUDE_PLUGIN_ROOT}/scripts/manage_remote.py" install <plugin>@<marketplace> [--scope user|local]
...
For enable/disable with smart name resolution and scope control, use `/cpv-enable-plugin` and `/cpv-disable-plugin`.
