...
<!--BADGES-START-->
![Version](https://img.shields.io/badge/version-5.18.1-blue)
...
| **Version-skew checker** | Read-only: compares an installed plugin's version against its marketplace-pinned version (semver skew none/major/minor/patch), with a best-effort CHANGELOG.md `BREAKING` hint | — (standalone script only) | `uv run python scripts/cpv_version_skew.py <installed-plugin-dir> <marketplace-ref> [--json]` |
