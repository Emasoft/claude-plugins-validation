...
RE_USED_VIA_THE_SKILLS_MENU = re.compile(
    r"(?:[Uu]sed|[Ll]oaded)\s+(?:dynamically\s+)?(?:by\s+\S+\s+)?via\s+(?:cpv-the-skills-menu|skills-index)",
    re.IGNORECASE,
)
...
def validate_frontmatter_structure(content: str, report: ValidationReport) -> dict[str, Any] | None:
    ...
    # Check frontmatter size (token budget). Extract the text BETWEEN the
    # opening and closing `---` delimiter LINES — not a bare `---` substring,
    # which would mis-measure a value that itself contains `---` (consistent
    # with parse_frontmatter above).
    _fm_lines = content.split("\n")
    _fm_close = next((k for k in range(1, len(_fm_lines)) if _fm_lines[k].strip() == "---"), None)
    fm_str = (
        "\n".join(_fm_lines[1:_fm_close])
        if (_fm_lines and _fm_lines[0].strip() == "---" and _fm_close is not None)
        else ""
    )
    fm_chars = len(fm_str)
        ...
        if is_user_invocable:
            ...
            if (
                ...
                report.minor(
                    ...
                    "'Used (dynamically) via cpv-the-skills-menu' / 'Loaded via cpv-the-skills-menu' "
                    "(TRDD-478d9687 / cpv-the-skills-menu method) so it's clear how agents "
                    "discover this skill.",
                    "SKILL.md",
                    category="Description Quality",
                )
