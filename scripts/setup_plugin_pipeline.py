...
# NOTE: setup_plugin_pipeline.py no longer ships a hardcoded GitHub Actions
# workflow template. v2.12.32 consolidated plugin CI into a single ci.yml
# with lint/validate/test jobs — that template is now only generated by
...
class PipelineSetup:
        ...
        if not has_validation_workflow:
            self.status.issues.append(
                PipelineIssue(
                    ...
                    fix_available=False,
                    fix_description=(
                        "Run `uv run python scripts/standardize_plugin.py . --fix` "
                        "to generate the consolidated .github/workflows/ci.yml "
                        "(lint + validate + test jobs) — v2.12.32 replaced the "
                        "old separate validate.yml with a single CI workflow."
                    ),
                )
            )
    ...
    def _fix_config_files(self) -> int:
        ...
        # GitHub workflow — do NOT write a hardcoded template here. The
        # canonical CI workflow is generated by scripts/generate_plugin_repo.py
        # (gen_ci_yml) and applied by scripts/standardize_plugin.py. Delegating
        # keeps one source of truth and avoids drift between this retrofit
        # tool and the consolidated CI layout introduced in v2.12.32.
        workflow_dir = self.project_path / ".github" / "workflows"
        ci_file = workflow_dir / "ci.yml"
        if not ci_file.exists():
            print(
                f"{YELLOW}Skipped:{NC} .github/workflows/ci.yml "
                f"— run `uv run python scripts/standardize_plugin.py "
                f"{self.project_path} --fix` to generate the consolidated "
                f"CI workflow."
            )
