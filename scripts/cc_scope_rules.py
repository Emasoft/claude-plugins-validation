...
KNOWN_SETTINGS_KEYS: frozenset[str] = frozenset(
    {
        ...
        # v2.1.233–235 sync backfill — every key below is a row of settings.md's
        # "Available settings" table (verified by mechanical set-diff against the
        # RAW doc, per the recorded spec-drift method; the only key NEW in this
        # window is `spellcheck`, v2.1.235 — the rest were accumulated drift).
        # This set stays TOP-LEVEL keys only; nested sub-keys are excluded.
        "advisorModel",
        "agent",
        ...
        "spellcheck",  # v2.1.235 — underline misspelled words in the prompt input (user/--settings/managed scopes)
        "sshConfigs",
