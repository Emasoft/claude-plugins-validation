...
def check_bom(content: bytes, file_path: str, report: EncodingValidationReport) -> bool:
    ...
    # UTF-32 LE BOM — must be checked before UTF-16 LE because \xff\xfe\x00\x00 starts with \xff\xfe
    ...
    # UTF-32 BE BOM — must be checked before UTF-16 BE because \x00\x00\xfe\xff starts with \x00\x00
...
"""
    ...
    if args.json:
        ...
        save_report_and_print_summary(
            report, Path(args.report), "Encoding Validation", _print_full, args.verbose, plugin_path=args.plugin_path
        )
    else:
