...
The cpv-plugin-creator-agent and cpv-plugin-fixer-agent agents MUST refuse to declare DONE on
...
flow now goes through the cpv-plugin-fixer-agent agent directly, dispatched from
...
class TestPluginCreatorPreflight:
    """Phase F.1 — cpv-plugin-creator-agent MUST gate on marketplace cross-check."""

    def test_plugin_creator_runs_marketplace_cross_check_before_declaring_done(self):
        """Look for explicit cross-check invocation in cpv-plugin-creator-agent.md."""
        text = (REPO_ROOT / "agents" / "cpv-plugin-creator-agent.md").read_text(encoding="utf-8")
        assert "validate_marketplace.py" in text, "cpv-plugin-creator-agent must reference validate_marketplace.py"
        # Phase F gate must explicitly cite the RC-MKPL-* MAJOR codes.
        assert "RC-MKPL-NAME-MISMATCH" in text, "cpv-plugin-creator-agent must call out RC-MKPL-NAME-MISMATCH as a block-on code"
        assert "RC-MKPL-UNKNOWN-FIELD" in text, "cpv-plugin-creator-agent must call out RC-MKPL-UNKNOWN-FIELD as a block-on code"
        assert "TRDD-c0ee9543" in text, "cpv-plugin-creator-agent gate must cite TRDD-c0ee9543"
...
class TestPluginFixerPreflight:
    """Phase F.2 — cpv-plugin-fixer-agent MUST gate on marketplace cross-check."""

    def test_plugin_fixer_blocks_completion_on_unresolved_marketplace_drift(self):
        text = (REPO_ROOT / "agents" / "cpv-plugin-fixer-agent.md").read_text(encoding="utf-8")
    ...
    def test_plugin_fixer_references_recipe_file(self):
        """cpv-plugin-fixer-agent must point at the recipes file for the codes it gates on."""
        text = (REPO_ROOT / "agents" / "cpv-plugin-fixer-agent.md").read_text(encoding="utf-8")
        assert "marketplace-upstream-drift.md" in text, "cpv-plugin-fixer-agent must reference the new recipe file"
...
class TestCpvUpgradePluginCommandDeletedInV290:
    ...
    The migration flow now goes through `agents/cpv-plugin-fixer-agent.md`
    ...
    def test_cpv_upgrade_plugin_command_file_is_deleted(self):
        ...
        assert not path.exists(), (
            ...
            "through cpv-plugin-fixer-agent agent dispatched from cpv-main-menu's "
    ...
    def test_plugin_fixer_agent_documents_marketplace_cross_check(self):
        ...
        cpv-plugin-fixer-agent agent (which is what the deleted command dispatched).
        """
        text = (REPO_ROOT / "agents" / "cpv-plugin-fixer-agent.md").read_text(encoding="utf-8")
        assert "validate_marketplace.py" in text
        assert "RC-MKPL-NAME-MISMATCH" in text
    ...
    def test_plugin_fixer_agent_references_ai_maestro_incident(self):
        """The migration gate's motivation citation moved into cpv-plugin-fixer-agent.md."""
        text = (REPO_ROOT / "agents" / "cpv-plugin-fixer-agent.md").read_text(encoding="utf-8")
...
class TestFixValidationSkillIndexes:
    """Phase F.4 — cpv-fix-validation skill must list the new RC-MKPL-* codes."""

    def test_marketplace_error_index_has_rc_mkpl_section(self):
        text = (REPO_ROOT / "skills/cpv-fix-validation/references/marketplace-error-index.md").read_text(encoding="utf-8")
    ...
    def test_plugin_error_index_has_rc_mkpl_section(self):
        text = (REPO_ROOT / "skills/cpv-fix-validation/references/plugin-error-index.md").read_text(encoding="utf-8")
    ...
    def test_skill_md_toc_parity(self):
        ...
        """
        skill_text = (REPO_ROOT / "skills/cpv-fix-validation/SKILL.md").read_text(encoding="utf-8")
        mkt_index = (REPO_ROOT / "skills/cpv-fix-validation/references/marketplace-error-index.md").read_text(
            encoding="utf-8"
        )
        plugin_index = (REPO_ROOT / "skills/cpv-fix-validation/references/plugin-error-index.md").read_text(
            encoding="utf-8"
        )
        ...
        assert "references" in skill_text.lower() or "reference" in skill_text.lower(), (
            "cpv-fix-validation SKILL.md must reference its references/ folder "
            "so cpv-plugin-fixer-agent can drill into per-error fix guides."
        )
        ...
        assert "marketplace-upstream-drift.md" in combined, (
            "cpv-fix-validation references/ must cite marketplace-upstream-drift.md in at least one error index."
        )
        assert "RC-MKPL-NAME-MISMATCH" in combined, (
            "RC-MKPL-NAME-MISMATCH must remain documented in cpv-fix-validation references."
        )
        assert "RC-MKPL-UNKNOWN-FIELD" in combined, (
            "RC-MKPL-UNKNOWN-FIELD must remain documented in cpv-fix-validation references."
        )

    def test_marketplace_upstream_drift_recipe_exists(self):
        """The recipe file itself must exist and have the 8 documented §s."""
        path = REPO_ROOT / "skills/cpv-fix-validation/references/marketplace-upstream-drift.md"
        assert path.is_file()
