...
- Finding 111 (skills/scaffold-agent/SKILL.md): the Output section used to
  claim the agent template emits ``model`` / ``maxTurns`` / ``skills``
  frontmatter fields. ``add_component._agent_template`` emits none of them.
- Finding 112 (skills/scaffold-command/SKILL.md): the Error-Handling table
...
- Finding 105 (skills/link-plugin-marketplace/SKILL.md): the doc used to
...
# ── Finding 112 — scaffold-command bare-Bash claim (two-sided) ────────────────
...
def test_bare_bash_in_allowed_tools_is_not_flagged():
    ...
    This is the exact value the scaffold-command default emits
...
def test_link_plugin_replaces_same_name_entry_with_fresh_metadata(tmp_path: Path):
    ...
    Guards the corrected link-plugin-marketplace doc: a second link of the
