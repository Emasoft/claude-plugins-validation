...
- Multiple drifts produce ONE WARNING listing every drifted file (so the
  user gets a single actionable summary, not eight separate warnings).
"""
...
def test_multiple_drifts_produce_single_consolidated_warning(tmp_path: Path) -> None:
    """Two drifted files → ONE warning listing both, not two separate warnings."""
    from cpv_validation_common import ValidationReport
    from validate_plugin import validate_canonical_pipeline_drift
    ...
    drift_warnings = [r for r in report.results if r.level == "WARNING" and "RC-PIPELINE-DRIFT-001" in r.message]
    assert len(drift_warnings) == 1, f"Expected 1 consolidated WARNING, got {len(drift_warnings)}"
    msg = drift_warnings[0].message
    assert "git-hooks/pre-push" in msg
    assert "scripts/publish.py" in msg
